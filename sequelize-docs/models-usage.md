...
当你从数据库中检索数据，很有可能你也想要同时获取关联信息-这被称作预加载。 其中的基本思想是， 当你调用 `find` 或 `findAll` 时使用 `include`属性。假设如下设置：
...
好的，所以，首先加载所有的 task 以及与它们关联的 user。
...
注意 accessor (结果实例中的 `User` 属性) 是单数形式因为一个 task 对应最多一个 user。
...
注意 accessor（结果实例中的 `Tasks` 属性）是复数形式因为一个 user 可以对应多个 task。


If an association is aliased (使用 `as` 选项)，你必须在 include 关联模型时指定别名。 注意下面的 user 关联的 `Tool` 是如何被重命名为 `Instruments` 的。为了使之正确，你必须同时指明你想要加载的模型和它的别名。
