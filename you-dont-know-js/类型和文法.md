...
## 2.1 Array
在一个array值上使用delete将会从这个array上移除一个值槽，但就算你移除了最后一个元素，它也 不会 更新length属性。要小心创建“稀散”的array（留下或创建空的/丢失的值槽）
### 2.1.1 类Array
1. 函数参数
2. dom 集合
...
## 2.2 String
string不可变性,改变array内容的方法实际上 是 原地修改的。
...
数字中的前置0x，0b，和0o分别表示 十六进制、二进制和八进制
